...
## ✅ DONE - Action Item 17: Save Preferences Per Book Set
**Status:** ✅ Complete
**Objective:** Persist user settings (font, margin, line-height, etc.) across page reloads so users don't lose their preferred reading conditions.
**Context:** Settings should be tied to the specific *collection* or *set* of books being read, meaning different book combinations can have different perfect reading setups.
**Architecture Sync:**
- A `collectionId` is dynamically generated by joining the active `urls` array (e.g., `reader_settings_/books/a.epub|/books/b.epub`).
- The `ParallelReader`'s `settings` state is strictly bound to this ID using `localStorage`—saving `onChange` and hydrating via a `useEffect` whenever the URL configuration changes.
**Tasks:**
- [x] Define a unique identifier for a book set (e.g., hashing or joining the 'urls' array).
- [x] Implement `localStorage` saving and loading for the `settings` state in `ParallelReader.tsx`.
